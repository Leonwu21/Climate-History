...
public class IceSheets extends AppCompatActivity {
    ...
    public void readFile(String filename1, String filename2, List<DataEntry> data) throws IOException {
        ...
        try {
            is1 = getAssets().open(filename1);
            is2 = getAssets().open(filename2);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < years.size(); i++) {
            data.add(new IceSheets.CustomDataEntry(years.get(i), massVar1.get(i), massVar2.get(i)));
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AnyChartView anyChartView = findViewById(R.id.is_graph);
